...
# ---------- EDIT THESE ----------
input_csv = "/data2/RANA/FINAL_RUN_7nc/PROPNANE_SENSITIVITY/PROPANE_MECH_data/all_propnae_data.txt"
...
#!/usr/bin/env python3
"""
One target per CSV row. Edit input_csv and run with python3.
"""
import os, re, math
import pandas as pd
...
input_csv = "/data2/RANA/FINAL_RUN_7nc/PROPNANE_SENSITIVITY/PROPANE_MECH_data/all_propnae_data.txt"
output_path = "targets.txt"
start_target_number = 10001001   # first target id -> x10001001
start_index = 1
# --------------------------
...
def format_line(index, target_id, T, P, phi_s, observed, comp):
    fuel_name = comp['fuel_name']
    fuel_s = f"{comp['fuel']:.8f}"
    o2_s   = f"{comp['O2']:.8f}"
    n2_s   = f"{comp['N2']:.8f}"
    ...
    obs_s = f"{observed:.1f}"
    return (
        f"{index}\t|{target_id}\t| target -- Tig\t| simulation -- Isochor Homo Reactor\t"
        f"| measurnment_type -- OHEX;max\t|Ignition_mode -- reflected\t| Flame_type -- \t"
        f"| Reactor_type -- \t| Fuel_type -- Multi\t"
        f"| Fuel -- x->{{'a': '{fuel_name}'}} = {{'a': {fuel_s}}}\t"
        f"| Oxidizer -- x->O2 = {o2_s}\t"
...
def read_csv_flex(path):
    ...
    cols = df.columns.tolist()
    x_col = next((c for c in cols if '1000' in c or '1000/x' in c or 'x (' in c.lower()), cols[0])
    y_col = next((c for c in cols if c.lower()=='y' or 'observed' in c.lower()), (cols[1] if len(cols)>1 else cols[0]))
    g_col = next((c for c in cols if 'group' in c.lower() or 'phi' in c.lower()), (cols[2] if len(cols)>2 else cols[-1]))
    def to_float(s): 
        return pd.to_numeric(s.astype(str).str.replace(',', '.').str.strip(), errors='coerce')
...
def main():
    ...
    lines = []
    idx = start_index
    target_num = start_target_number
    for _, row in df.iterrows():
        x = row['x_trans']; y = row['y']; g = row['group']
        if pd.isna(x) or pd.isna(y):
            continue
        phi, P = parse_phi_P(g)
        comp = find_phi_comp(phi) if phi is not None else None
        if comp is None:
            comp = {"fuel_name": "C3H8", "fuel": 0.0, "O2": 0.0, "N2": 0.0}
        phi_s = f"{phi:.1f}" if phi is not None else ""
        line = format_line(idx, f"x{target_num}", float(x), P, phi_s, float(y), comp)
        lines.append(line)
        idx += 1
        target_num += 1
        written += 1
    ...
    print(f"Total rows in CSV: {total_rows}")
    print(f"Wrote {written} target lines to {output_path}")
