...
for case in case_dir:	
	...
	if "sim_data_case-"+str(case)+".lst" in os.listdir():
		ETA_x2 = [i.split("\t")[1] for i in open("sim_data_case-"+str(case)+".lst").readlines()]
...
for case in case_dir:
	...
	species = []
	sa_dict = {}
	for count,i in enumerate(multiply):
	    if str(i).strip("\n") != "None":
	        case_FM.append(((np.array([float(i)]) - nominal)/nominal).flatten()[0])
	        species.append(index[count])
	        #sa_dict[index[count]] = ((np.array([float(i.strip('\n'))]) - nominal)/nominal).flatten()[0]
	        sa_dict[index[count]] = ((np.array([float(i)]) - nominal)/nominal).flatten()[0]
	#case_FM = (multiply - nominal)/nominal
	#print(reaction_dict,index)
	#print(case_FM)
	sensitivity_FM = dict(sorted(sa_dict.items(), key=lambda item: abs(item[1]),reverse = True))
