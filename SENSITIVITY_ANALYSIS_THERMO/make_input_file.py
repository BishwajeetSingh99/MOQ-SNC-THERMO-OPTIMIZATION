...
""".format(
    ...
    dpdt = target.add.get("dpdt"),
    dt = float(target.add.get("dt", 0.0)),
    estimateTIG = float(target.add.get("estimateTIG", 0.0)),
    delay_def = target.add["ign_delay_def"],
    delay_cond = target.add["ign_cond"],
    specific_cond = target.add["specific_cond"],
    saveAll=target.add.get("saveAll")
)
...
""".format(mech = mech_file,temperature_K = target.temperature,pressure=target.pressure,species_conc = target.species_dict,exp_conc = target.add["exp_conc"][float(target.temperature)],End_Time = target.add["EndTime"], delay_def = target.add["ign_delay_def"],delay_cond = target.add["ign_cond"],specific_cond = target.add["specific_cond"],saveAll=target.add["saveAll"])
...
python3.10 {MUQSAC}/soln2ck_2.py --mechanism=mechanism.inp --thermo=thermo.dat {mech} &> soln2ck.out
python3.10 cantera_.py &> solve
		...
		s_convert = """#!/bin/bash
python3.10 /home/krithika/Desktop/KineticMechanismOptimization/sc_v2/v2.1/soln2ck.py &>sol2yaml_out
