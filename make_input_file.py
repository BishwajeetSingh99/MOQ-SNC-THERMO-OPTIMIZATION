...
def ignitionDelay(df,pList, species,cond="max",specified_value ="None;None",exp_conc = "None"):
	...
	elif cond == "dt-max" and species == "T":
		if "temperature" in df.columns:
			signal = df["temperature"].to_numpy()
		elif "T" in df.columns:
			signal = df["T"].to_numpy()
		else:
	...
	elif cond == "dt-max" and species != "p" and species != "T":
		time = np.diff(df.index.to_numpy())
...
rm -f ScanMan.log
		...
		s_run = """#!/bin/bash
python3.10 {MUQSAC}/soln2ck_FM.py --mechanism=mechanism.inp --thermo=therm.dat --transport=trans.dat {mech} &> soln2ck.log 
