...
class ResponseSurface(object):
	...
	def test(self,xTest,yTest):
		...
		if self.ytestMaxError > 2:#if maximum error in PRS testing is greater than 2 percentage, the response surface is rejected
			...
			self.selection = 1
		rr = open(os.getcwd()+'/Data/ResponseSurface/selection_case-'+str(self.case_index)+'.csv','w').write(str(self.selection))	
	...
	def plot(self,index):
		...
		#plt.plot(np.asarray(self.y_Test_simulation),np.asarray(self.y_Test_Predict),"r.",ms=8,label=f"Testing (max error = {self.ytestMaxError :.3f}%, mean error = {self.yTestMeanError:.3f}%)")
		#plt.scatter(np.asarray(yData_testing), np.asarray(Sim_value_testing), color="none", edgecolor="black")
		#plt.scatter(np.asarray(self.Y), np.asarray(self.resFramWrk), color="black", edgecolor="black",label=f"Training (max error = {self.MaxError:.3f}%, mean error = {self.MeanError:.3f}%)")
		...
		plt.savefig('../Plots/Parity_plot_case_'+str(self.case_index)+'_TESTING.pdf',bbox_inches="tight")	
	'''
	...
	def plot(self,index):
		...
		# Set the exact frame border (spine) colors and thickness
		for spine in ax.spines.values():
		    spine.set_edgecolor('black')
		    spine.set_linewidth(0.7) 
		...
		# Plot the Testing data as black dots ('k.') with size 8
		plt.plot(np.asarray(self.y_Test_simulation), np.asarray(self.y_Test_Predict), "k.", ms=8, 
		         label=f"Testing (max error = {self.ytestMaxError:.3f}%, mean error = {self.yTestMeanError:.3f}%)")
		
		# Generate the ideal parity reference line
		min_val = min(np.asarray(self.Y))
		max_val = max(np.asarray(self.Y))
		x = np.linspace(min_val * 0.95, max_val * 1.05, 1000)
		
		# Plot the parity line as a solid RED line ('r-')
		plt.plot(x, x, "r-", label="Parity line")
		
		# Set exact axis limits to fit data symmetrically like the PDF
		plt.xlim(min_val * 0.99, max_val * 1.01)
		plt.ylim(min_val * 0.99, max_val * 1.01)
		
		# Remove grid lines completely
		ax.grid(False)
		
		# Place the legend at the top-left inside the frame bounds
		plt.legend(loc="upper left")
	...
	def create_response_surface(self):
		...
		if 'responsecoef_case-'+str(self.case_index)+'.csv' in os.listdir(os.getcwd()+'/Data/ResponseSurface/'):
			print(f"The PRS coeff file is generated for case-{self.case_index}") 
			f = open(os.getcwd()+'/Data/ResponseSurface/responsecoef_case-'+str(self.case_index)+'.csv','r').readlines()
			g = open(os.getcwd()+'/Data/ResponseSurface/selection_case-'+str(self.case_index)+'.csv','r').readlines()
			self.coeff = np.asarray([float(i) for i in f[1:]])
			self.selection = [int(i) for i in g][0]
			self.generated = True
		else:
			self.BTrsMatrix = np.asarray(self.MatPolyFitTransform())
			
			# Compute raw Normal Equation components
			Phi_T_Phi = np.dot(self.BTrsMatrix.T, self.BTrsMatrix)
			Phi_T_y = np.dot(self.BTrsMatrix.T, self.Y)
			
			# Create the scaling diagonal vector to normalize feature scales
			scale_diagonal = np.sqrt(np.diag(Phi_T_Phi))
			scale_diagonal[scale_diagonal < 1e-12] = 1.0
			
			# Scale matrix components symmetrically
			Scaled_Phi_T_Phi = Phi_T_Phi / np.outer(scale_diagonal, scale_diagonal)
			Scaled_Phi_T_y = Phi_T_y / scale_diagonal
			
			# Add Tikhonov (Ridge) regularization floor to eliminate singularity
			alpha = 1e-5
			Identity_Patch = alpha * np.eye(Scaled_Phi_T_Phi.shape[0])
			
			# Solve the stabilized system safely
			scaled_coeff = np.linalg.solve(Scaled_Phi_T_Phi + Identity_Patch, Scaled_Phi_T_y)
			
			# Transform coefficients back to the original physical parameter space
			self.coeff = scaled_coeff / scale_diagonal
