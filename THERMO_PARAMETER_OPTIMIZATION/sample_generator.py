...
def sample_plot_all_species(unsrt_data, design_matrix, outdir="sample_plots", seed=None):
    ...
    for row_idx in chosen_rows:
        ...
        for sp_idx, sp in enumerate(species_order):
            ...
            # 1. Clean Background & Solid Border Box
            ...
            # 2. Inward Ticks on All Sides (Standard Scientific Style)
            ax.tick_params(axis='both', which='both', direction='in', top=True, right=True, labelsize=10)

            # 3. Plots
            ax.plot(T_lo, cp_lo, label="Sample (<1000 K)",  color="red", linewidth=1.5)
            ax.plot(T_hi, cp_hi, label="Sample (>1000 K)", color="green", linewidth=1.5)

            ax.plot(T_lo, c["nom_lo"], color="blue", label="Nominal", linewidth=2.0)
            ax.plot(T_hi, c["nom_hi"], color="blue", linewidth=2.0)

            ax.plot(T_lo, c["env_lo_hi"], "k--", label="Uncert. limits", linewidth=1.2)
            ax.plot(T_lo, c["env_lo_lo"], "k--", linewidth=1.2)
            ax.plot(T_hi, c["env_hi_hi"], "k--", linewidth=1.2)
            ax.plot(T_hi, c["env_hi_lo"], "k--", linewidth=1.2)

            # 4. Clear and Highly Visible Header
            ax.set_title(f"Species: {sp} (Row {row_idx})", fontsize=12, fontweight='bold', pad=12)

            # 5. Label and Legend Quality Tweaks
            ax.set_xlabel("Temperature (K)", fontsize=11, labelpad=8)
            ax.set_ylabel(r"$C_p$ (J mol$^{-1}$ K$^{-1}$)", fontsize=11, labelpad=8)
            ax.set_ylim(0.92 * c["env_lo_lo"].min(),
                        1.15 * c["env_hi_hi"].max())
            
            # Boxed legend with an explicit crisp outline
            ax.legend(fontsize=9.5, loc="upper right", frameon=True, facecolor='white', edgecolor='black')
            ax.grid(False)
